...
describe('Unprotected response', () => {
  it('Checks whether the rows in the participant.tsv file generated according to session_type', () => {
    ...
    readLatestFile('cypress/downloads/neurobagel-query-results_*.tsv').then((fileContent) => {
      ...
      expect(phenotypicSession.split('\t')[15]).to.equal(
        'Blood-Oxygen-Level Dependent image,T2-weighted image'
      );
      ...
      expect(imagingSession.split('\t')[13]).to.equal(
        'Blood-Oxygen-Level Dependent image,T2-weighted image'
      );
      expect(imagingSession.split('\t')[14]).to.equal('fmriprep 23.1.3,freesurfer 7.3.2');
      expect(imagingSession.split('\t')[15]).to.equal(
        'Blood-Oxygen-Level Dependent image,T2-weighted image'
      );

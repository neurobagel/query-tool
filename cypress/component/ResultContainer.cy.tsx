...
describe("ResultContainer", () => {
    it("Displays a set of Result Cards, select all checkbox, disabled download result buttons, summary stats, and how to get data modal button", () => {
        ...
        cy.get('[data-cy="summary-stats"]').should("be.visible").should("contain", "Summary stats: 2 datasets, 4 subjects");
        cy.get('[data-cy="participant-level-download-results-button"]').should("be.visible").should("be.disabled");
        cy.get('[data-cy="dataset-level-download-results-button"]').should("be.visible").should("be.disabled");
    ...
    it('Selecting a dataset should enable the download result buttons', () => {
        ...
        cy.get('[data-cy="participant-level-download-results-button"]').should("be.visible").should("not.be.disabled");
        cy.get('[data-cy="dataset-level-download-results-button"]').should("be.visible").should("not.be.disabled")
    });
    it("Selecting/unselecting select all datasets checkbox should check/uncheck all dataset cards", () => {
        ...
        cy.get('[data-cy="card-http://neurobagel.org/vocab/cool-dataset-checkbox"] input').should("be.checked");
        cy.get('[data-cy="card-http://neurobagel.org/vocab/not-so-cool-dataset-checkbox"] input').should("be.checked");
        cy.get('[data-cy="select-all-checkbox"] input').uncheck();
        cy.get('[data-cy="card-http://neurobagel.org/vocab/cool-dataset-checkbox"] input').should("not.be.checked");
        cy.get('[data-cy="card-http://neurobagel.org/vocab/not-so-cool-dataset-checkbox"] input').should("not.be.checked");
    });
    ...
    it("Shows no result view when result is empty", () => {
        ...
        cy.get('[data-cy="empty-result-container-view"]').should("be.visible").should("contain", "No results");
    })
    it("Shows Click Submit Query view when result is null", () => {
        cy.mount(<ResultContainer result={null} />);
        cy.get('[data-cy="default-result-container-view"]').should("be.visible").should("contain", "Click 'Submit Query' for results");
    })
